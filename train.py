...
# device = torch.device("cuda" if torch.cuda.is_available() else "cpu") #只能避开CPU的使用
...
parser.add_argument('--dataset', type=str, default='twitter16',
                    help='the dataset name: twitter15, twitter16(default: twitter16)')
...
parser.add_argument('--elapsed_time',type=int, default= 3000,
                    help='the elapsed time after source tweet posted (0, 60(1h), 120(2h), 240(4h), 480(8h), 720(12h), 1440(24h), default: 3000 represents all)')
parser.add_argument('--tweets_count', type=int, default=50, 
                    help='the tweets count after source tweet posted (10, 20, 30, 40, 50, default: 500 represents all)')
parser.add_argument('--batch_size', type=int, default=128,
                    help='input batch size for training (default: 128/180)')
...
def train(epoch, best_acc, patience):
    ...
    plt.plot(epochs, average_acc, 'r', label='validation accuracy')
    plt.title('Training and validation accuracy')
    plt.legend(loc='lower right')
    plt.figure()

    plt.plot(epochs, loss, 'r', label='Training loss')
    plt.plot(epochs, average_loss, 'b', label='validation loss')
